...
/* 
	...
	# 스프링 로그파일 설정 방법 
		...
		- 기본적으로 아래의 의존성에 logback이 구현되어 있음
		'org.springframework.boot:spring-boot-starter-web' 
		
		
		2) src/main/resources경로 아래 logback.xml 파일 생성 및 설정 log4j.xml 설정 파일에서 appender 설정
...
public class LoggerAdvice {
	...
	/* 
		...
		import org.slf4j.LoggerFactory;
	 	...
	 	- 여러개발자가 협업을 하는 경우 logback 로깅방식을 통해서 로깅방법을 통일할 수 있습니다.
	 	- logback을 사용하여 로그의 레벨별로 체계적인 로그관리가 가능합니다.
	 	- 로깅 레벨을 설정하면 그 이상 레벨을 로깅합니다.
	 	- Spring Boot는 기본적으로 INFO 레벨로 설정되어 있습니다. 
	 	
	 	
		level 6) FATAL:
		몇몇 로깅 프레임워크에서만 지원하는 레벨로, 치명적인 오류를 나타냅니다.

		level 5) ERROR:
		에러 레벨은 치명적인 오류 메시지를 기록합니다. 애플리케이션은 계속 실행될 수 있지만, 심각한 문제가 발생했음을 나타냅니다.
		
		level 4) WARN:
		경고 레벨은 경고 메시지를 기록합니다. 어떤 문제가 발생했지만 애플리케이션은 계속 실행될 수 있습니다.
		
		level 3) INFO:
		일반적인 실행에 대한 정보를 기록합니다. 애플리케이션의 주요 이벤트 및 상태 변경에 대한 정보를 보여줍니다.
		
		level 2) DEBUG:
		디버깅에 유용한 로그를 기록합니다. 개발 중에 자세한 정보를 확인할 때 사용됩니다.
		
	 	level 1) TRACE:
		가장 낮은 로깅 레벨로, 매우 상세한 로그를 기록합니다. 개발 중 디버깅에 사용됩니다.
	...
	*/	
	...
	@Around("execution(public void com.spring.aop.log.*.*(..))")
