...
/*
	...
	# AOP ( Aspect-Oriented Programming ) 관점 지향 프로그래밍
		...
		- 프로젝트 개발 과정에서 핵심 기능 외에 추가적이고 다양한 부가(공통) 기능이 필요합니다. (로깅 , 보안 , 트랜잭션 , 테스트 , 등등)
		- 이 부가(공통)기능들은 프로젝트에 중요한 역할을 하며 이 부가(공통)기능이 코드마다 반복적(중복)으로 나타나는 부분이 존재합니다. 
		- 코드에서 비즈니스 핵심 로직과 부가기능을 분리하여 부가 로직을 따로 관리(모듈화)합니다.
		- 종단(비즈니스 로직) 기능 , 횡단(관심,Aspect) 기능 
		- 부가 기능이 비즈니스 로직(핵심 기능)을 담은 클래스의 코드에 전혀 영향을 주지 않으면서 부가기능의 구현을 용이하게 할 수 있는 구조를 제공합니다.
		- AOP는 OOP를 대체하는 새로운 개념이 아니라 OOP를 돕는 보조적 기술 중에 하나입니다.
		...
		[ 용어 정리 ]
			...
			2) Advice	   : 핵심기능에 부여되는 부가기능 ( 타겟 메서드에 적용될 부가 기능 )
				...
				2-1) Around (Advice)		 : 대상 객체의 메서드 실행 전 , 후 및 예외 발생 모두 실행합니다.
				2-2) Before (Advice)	     : 대상 객체의 메서드 메서드 호출전에 수행합니다.
				2-3) After (Advice)			 : 대상 객체의 메서드 실행도중 예외 발생 여부와 상관없이 메서드 실행 후 실행합니다.
				2-4) AfterReturning (Advice) : 대상 객체의 메서드가 실행 도중 예외없이 실행 성공한 경우에 실행합니다.
				2-5) AfterThrowing (Advice)  : 대상 객체의 메서드가 실행 도중 예외가 발생한 경우에 실행합니다.
...
public class AOPAdvice {
	...
	@Pointcut("execution(public void com.spring.aop.ex.*.work())") // 중복되는 execution명시자를 작성합니다.
	public void abcde() {
		// 특정의미가 없다.
	}
	...
	public void beforeWork() {
		System.out.println("(before)출근합니다.");
	}
	
	// 메서드 호출후
	@After("abcde()")
	public void afterWork() {
		System.out.println("(after)퇴근합니다.\n");
	}
	...
	public void aroundGetWorkingTime(ProceedingJoinPoint pjp) throws Throwable {
		...
		// ProceedingJoinPoint 객체의 proceed(); 메서드를 호출하여 타겟팅 메서드를 실행합니다.
		pjp.proceed();
	...
	// 타겟팅메서드가 성공적으로 실행된 후
	@AfterReturning("execution(public void com.spring.aop.ex.*.getInfo(..))")
	//          import org.aspectj.lang.JoinPoint;
	public void afterReturningGetInfo(JoinPoint jp) { // JoinPoint를 통하여 메서드의 파라메타를 전달받을 수 있습니다.
	...
	// 타겟팅 메서드에서 오류 발생 후
	@AfterThrowing("execution(public void com.spring.aop.ex.Boss.getError())")
